...
class VectorStore:
    ...
    def _build_chunk_id(
        ...
        filename = Path(str(doc_meta.get("filename") or doc_meta.get("source") or "document")).stem
    ...
    def add_chunks(
        ...
        for i, chunk in enumerate(chunks):
            chunk_id = self._build_chunk_id(doc_meta, chunk, i, used_ids)
            ids.append(chunk_id)
            ...
            metadatas.append({k: str(v) if v is not None else "" for k, v in metadata.items()})
    ...
    def search(
        ...
        where = {k: str(v) for k, v in filter_metadata.items() if v} if filter_metadata else None
